...
export default function OpenDartsLeaguePage() {
  ...
  useEffect(() => {
    fetch("/api/league-tables")
      .then((res) => res.json())
      .then((data) => {
        const division = data.divisions.find((d: any) => d.name === "Open Darts");
  ...
  return (
    <main className="max-w-2xl mx-auto p-4 bg-white text-black">
      <h1 className="text-3xl font-bold mb-6 text-center">Open Darts League Table</h1>
      {loading ? (
